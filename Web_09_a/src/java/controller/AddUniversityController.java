...
public class AddUniversityController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UniversityDTO udto = udao.searchByID(id);
            if (udto != null) {
            ...
            try {
                ...
                if (totalStudents < 0) {
                    error += ("Tong so sinh vien la so nguyen duong <br/>");
                }
            } catch (Exception e) {
            ...
            try {
                ...
                if (totalFaculties < 0) {
                    error += ("Tong so nhan vien la so nguyen duong <br/>");
                }
            } catch (Exception e) {
                error += ("tong so nhan vien phai la so nguyen<br/>");
            }
            ...
            if (error.isEmpty()) {
                udto = new UniversityDTO(id, name, shortName, description, foundedYear, address, city, region, type, totalStudents, totalFaculties, isDraft);

                //khong co loi
                if (udao.add(udto)) {
                    msg = "Da them univrtsity thanh cong!";
                } else {
                    error += "Gap loi , khong the them University!";
                    request.setAttribute("u", udto);
                    // request để gửi lên trang cho người dùng chỉnh sửa cho hợp lý
                }
